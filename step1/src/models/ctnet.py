...
class CTNet(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.params = params
        n_outputs   = self.params['parameters']['n_outputs']
    ...
    def load(self):
        self.load_state_dict(torch.load(self.params['model']['path_ctnet'], map_location=self.params['device']))
...
class CTNetStep(nn.Module):
    ...
    def __init__(self, params):
        ...
        # pre-trained backbone : CTNet
        self.encoder = CTNet(params)
        if self.params['model']['flag_pretrain_ctnet']:
            self.encoder.load()
        self.encoder.adjust()

        # classification heads : one per label for multi-task single label
    ...
    def forward(self, volume, labels, return_embeddings=False):
        ...
        if return_embeddings:
            return hidden_states, predictions, loss.mean(dim=0)
        else:
            return predictions, loss.mean(dim=0)
